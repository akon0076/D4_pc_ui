...
<template>
    <div class="spanControl-list" style="width: 100%;margin-left: 0!important;">
        <el-card class="box-card card-head" style="margin: 0px; width: 100%; min-height: 99%">
            ...
            <div class="text item">
                <el-form ref="testEmployeeForm" :model="testEmployee" label-width="150px" :rules="rules">
                                    <el-col :span="12">
                                        <el-form-item label="名称" prop="name">
                                            <el-input type="input" v-model="testEmployee.name"
                                    ...
                                    <el-col :span="12">
                                        <el-form-item label="员工姓名" prop="staffName">
                                            <el-input type="input" v-model="testEmployee.staffName"
                                                      placeholder="名称" clearable autosize
                                    ...
                                    <el-col :span="12">
                                        <el-form-item label="身份证号" prop="staffIdNumber">
                                            <el-input type="input" v-model="testEmployee.staffIdNumber"
                                ...
                                <el-col :span="12">
                                    <el-form-item label="年龄" prop="staffAge">
                                        <el-input type="number" step="1" v-model="testEmployee.staffAge"
                                ...
                                <el-col :span="12">
                                    <el-form-item label="薪水" prop="salary">
                                        <el-input type="number" step="0.01" v-model="testEmployee.salary"
                                ...
                                <el-col :span="12">
                                    <el-form-item label="分数" prop="score">
                                        <el-input type="number" step="1" v-model="testEmployee.score"
                                ...
                                <el-col :span="12">
                                    <el-form-item label="入职日期" prop="dateOfJoin">
                                        <el-date-picker clearable
                                                v-model="testEmployee.dateOfJoin"
                                                align="right"
                                                type="date"
                                                placeholder="入职的时间"
                                ...
                                <el-col :span="12">
                                    <el-form-item label="单位" prop="organizationName">
                                        <el-autocomplete
                                                class="inline-input"
                                                value-key="name"
                                                v-model="testEmployee.organizationName"
                                ...
                                <el-col :span="12">
                                    <el-form-item label="是否有效" prop="enabled">
                                        <el-radio-group v-model="testEmployee.enabled">
                                ...
                                <el-col :span="24">
                                    <el-form-item label="备注" prop="remark">
                                            ...
                                            <el-input type="textarea" v-model="testEmployee.remark"
                    ...
                    <el-col :span="12">
                        <el-form-item>
                            <el-button type="primary" @click="submitTestEmployee()"    :loading="isSubmiting" v-permission:simple_organization_TestEmployee_Edit >提交</el-button>
                        </el-form-item>
                    </el-col>
                </el-form>
            </div>
...
export default {
    ...
    methods: {
        submitTestEmployee() {
            var refs = this.$refs;
            refs['testEmployeeForm'].validate(valid => {
            if (valid) {
            if(this.testEmployee.eid)//编辑测试表单
            {
            this.updateTestEmployee();
            }
            else//保存测试表单
            {
            this.saveTestEmployee();
            }
            } else {
            return false
            }
        ...
        saveTestEmployee()//保存测试表单
        {
            this.isSubmiting = true;
            TestEmployeeService.saveTestEmployee(this.testEmployee).then((resp) => {
                this.isSubmiting = false;
                var router = this.$router;
                router.push({path: '/simple/testEmployee'})
        }).catch((error) => {
            this.isSubmiting = false;
            this.$message({
            type: 'error',
        ...
        updateTestEmployee()//编辑测试表单
        {
            this.isSubmiting = true;
            TestEmployeeService.updateTestEmployee(this.testEmployee).then((resp) => {
                this.isSubmiting = false;
                var router = this.$router;
                router.push({path: '/simple/testEmployee'})
        }).catch((error) => {
        ...
        findTestEmployeeForEdit(testEmployeeId)//查找测试表单
        {
            TestEmployeeService.findTestEmployeeForEdit(testEmployeeId).then((resp) => {
        ...
        createTestEmployee()//创建新的测试表单
        {
            TestEmployeeService.createTestEmployee().then((resp) => {
                this.prepareForEdit(resp.data);
        }).catch((error) => {
            this.$message({
            type: 'error',
        ...
        prepareForEdit(testEmployeeEditDto)
        {
            this.testEmployee = testEmployeeEditDto.testEmployee;

            this.genderCodeTables = testEmployeeEditDto.genderCodeTables;
            this.organizationOrganizations = testEmployeeEditDto.organizationOrganizations
        },
        ...
        searchOrganizationName(queryString, cb) {
            ...
            handleSelectOrganizationName(item){
                this.testEmployee.organizationId = item.eId;
            },
    },
    created() {
        this.testEmployeeId = this.$route.params.testEmployeeId;
        if(this.testEmployeeId)//编辑
        {
            this.findTestEmployeeForEdit(this.testEmployeeId);
        }
        else//新增
        {
