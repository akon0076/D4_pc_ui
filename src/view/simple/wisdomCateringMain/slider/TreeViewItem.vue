  ...
  <div class="tree-view-item">
    <div class="level" :class="'level-'+ menu.level" v-for="menu in menus" :key="menu.id">
      <div v-if="menu.nodes == null || menu.nodes.length == 0">
        <router-link class="link" v-bind:to="menu.routeData" @click.native="toggle(menu)">{{menu.text}}</router-link>
      </div>
      <div v-if="menu.nodes != null && menu.nodes.length != 0">
        ...
        <transition name="fade">
          <div class="heading-children" v-show="menu.isExpanded" v-if="menu.nodes">
            <Tree-view-item :menus='menu.nodes'></Tree-view-item>
          </div>
        </transition>
      </div>
    </div>
